...
const cookieOptions = (isProduction = process.env.NODE_ENV === 'production') => ({
  ...
  path: '/' // Important : spécifier le path
});
...
const setTokenCookies = (res: Response, tokens: { accessToken: string; refreshToken: string }) => {
  res.cookie('accessToken', tokens.accessToken, {
    ...cookieOptions(),
    maxAge: 24 * 60 * 60 * 1000 // 24 heures au lieu de 15 minutes
  });
...
export const signup = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const userWithoutPassword = {
      userId: user._id, // ✅ Utiliser userId au lieu de _id
      name: user.name,
      email: user.email,
      role: user.role,
      image: user.image,
...
export const login = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const userWithoutPassword = {
      userId: user._id, // ✅ Cohérence avec signup
      email: user.email,
      role: user.role,
      name: user.name,
      image: user.image
...
export const oauthSuccess = async (req: Request & { user?: OAuthUser }, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!req.user || typeof req.user !== 'object') {
      ...
      res.redirect(`${process.env.CLIENT_URL || 'http://localhost:3000'}/sign-in?error=authentication_failed`);
      return;
    }
    ...
    if ('id' in req.user) {
      ...
      res.redirect(`${process.env.CLIENT_URL || 'http://localhost:3000'}/sign-in?error=invalid_user_format`);
      return;
    }
    
    if (!userId || !email) {
      console.error('OAuth callback: Données utilisateur incomplètes', req.user);
      res.redirect(`${process.env.CLIENT_URL || 'http://localhost:3000'}/sign-in?error=incomplete_user_data`);
      return;
    }
    ...
    // ✅ Utiliser la fonction standardisée pour les cookies
    setTokenCookies(res, tokens);
